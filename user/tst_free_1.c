...
void _main(void)
{
	...
	cprintf("\n%~[2] Free all the allocated spaces from PAGE ALLOCATOR \[70%]\n");
	{
		//Free 1st 2 MB
		{
			...
			if ((usedDiskPages - sys_pf_calculate_allocated_pages()) != 0) { is_correct = 0; cprintf("9 Wrong free: Extra or less pages are removed from PageFile %d \n",(usedDiskPages - sys_pf_calculate_allocated_pages()) );}
			if ((sys_calculate_free_frames() - freeFrames) != 2 ) { is_correct = 0; cprintf("9 Wrong free: WS pages in memory and/or page tables are not freed correctly %d \n",(sys_calculate_free_frames() - freeFrames));}
