...
int process_command(int number_of_arguments, char** arguments)
{
	...
	{
		if(isSubString(arguments[0] , commands[i].name) == 1)
		{
...
int isSubString(char *str1 , char *str2){
	while(1==1){
		//str1 "points to 1st char in string" (compare each char in substring(str1) with superstring(str2))
		if(*str1 == *str2){ // "clock" -> "clk"
			str1++; //move pointer to the next char in sub
		}
		str2++;

		if (*str1 == '\0')
			return 1;    //all chars in str1 found in str2 with same order

		if(*str2 == '\0')//str2 finished before all of str1 chars found
			return 0;
	}
	return 0;
}
