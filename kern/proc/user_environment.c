...
void* create_user_kern_stack(uint32* ptr_user_page_directory) {
	...
	struct Env *va = kmalloc(KERNEL_STACK_SIZE);
	unmap_frame(ptr_user_page_directory, (uint32)&(va->kstack));
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart,uint32 daLimit) {
	...
	e->heapHardLimit = daLimit;
	e->heapStart = daStart;
	e->heapBreak = daStart;


//	cprintf("initialize_uheap: start of heap %x hard Limit %x\n" , daStart , daLimit);

//	cprintf("initialize_uheap_dynamic_allocator: free_block_list size is  %d \n" , LIST_SIZE(&freeBlocksList));

	initialize_dynamic_allocator(daStart , 0);

//	cprintf("initialize_uheap: start of heap %x hard Limit %x\n" , e->heapStart , e->heapHardLimit);
...
void initialize_environment(struct Env* e, uint32* ptr_user_page_directory,
	...
	initialize_uheap_dynamic_allocator(e, USER_HEAP_START,
...
void cleanup_buffers(struct Env* e) {
	...
	acquire_spinlock(&MemFrameLists.mfllock);
	{
		...
		{
			if (ptr_fi->proc == e) {
				pt_clear_page_table_entry(ptr_fi->proc->env_page_directory,
						ptr_fi->bufferedVA);
