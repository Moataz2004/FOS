...
void params_vlaidation(uint32 virtual_address) {
	if (virtual_address == (uint32) NULL
...
void sys_allocate_user_mem(uint32 virtual_address, uint32 size) {
	...
	params_vlaidation(virtual_address);
...
/*******************************/
//TODO:[PROJECT'24.MS3] ADD SUITABLE CODE HERE
void sys_create_semaphore(char *semaphoreName, uint32 value,
...
void sys_get_semaphore(int32 ownerEnvID, char* semaphoreName) {
...
void sys_wait_semaphore(struct __semdata* sem) {
	...
	acquire_spinlock((struct spinlock*) &sem->lock);
	sem->count--;

	if (sem->count < 0) {
		enqueue(&(sem->queue), env);
		acquire_spinlock(&ProcessQueues.qlock);
		release_spinlock((struct spinlock*) &sem->lock);
...
void sys_signal_semaphore(struct __semdata *sem) {
	...
	acquire_spinlock((struct spinlock*) &sem->lock);
	sem->count++;
	if (sem->count <= 0 && queue_size(&(sem->queue)) > 0) {
...
int sys_semaphore_count(struct semaphore *sem) {
	return (*sem).semdata->count;
}
/*******************************/
/* Priority RR Scheduler SYSTEM CALLS */
/*******************************/
//TODO:[PROJECT'24.MS3] ADD SUITABLE CODE HERE
void sys_env_set_priority(int32 envID, int priority) {
	...
	if (envid2env(envID, &target_env, 1) != 0) {
		cprintf("Error: Invalid environment ID %d\n", envID);
		return;
	}
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4,
	...
	case SYS_utilities:
		...
		return 0;
		//TODO: MS3 - semaphores sysCalls
	case SYS_create_semaphore:
		sys_create_semaphore((char *) a1, (uint32) a2, (struct semaphore*) a3);
		break;
	case SYS_get_semaphore:
		sys_get_semaphore((int32) a1, (char *) a2);
		break;
	...
	case SYS_signal_semaphore:
		...
		break;
	case SYS_get_semaphore_count:
		return sys_semaphore_count((struct semaphore*) a1);
		break;
		//TODO: MS3 - priority RR scheduler sysCalls
