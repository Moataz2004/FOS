...
//	cprintf("WorkingSetElement : New WS element allocated at address = %x and VA = %x ,eva = %x \n", element , virtual_address ,element->virtual_address);
	if (element == NULL) {
	    panic("Failed to allocate memory for WS element!");
	}
	return element;
}
...
//		    	cprintf("env_page_ws_invalidate : WS va =%x was removed\n",wse->virtual_address);
